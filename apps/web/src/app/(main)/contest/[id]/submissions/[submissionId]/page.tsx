      ...
      <div style={{ marginBottom: 32 }}>
        <Descriptions bordered column={1} size="small">
          <Descriptions.Item label="提出者">
            <Link href={`/user/${submission.user.id}`} className="flex items-center gap-2">
              <Avatar
                size="small"
                src={submission.user.image}
                icon={!submission.user.image && <UserOutlined />}
              />
              <Text className="hover:underline">{submission.user.name || 'Unknown'}</Text>
            </Link>
          </Descriptions.Item>
          ...
          <Descriptions.Item label="問題">
            <Link href={`/contest/${contestId}/problem/${submission.problem.id}`}>
              <Text className="hover:underline">{submission.problem.title}</Text>
            </Link>
          </Descriptions.Item>
